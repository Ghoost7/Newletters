...
export const validateEmail = yup.string().email().label("E-mail")
...
export const validateUsername = yup
  ...
  .matches(
    /^[a-z][a-z0-9._]?$/,
    "Username must contain only letters and numbers and '.' and '_' "
  )
  .trimm()
  .label("Username")
...
export const validateLimit = yup
  .number()
  .min(1)
  .max(config.view.results.minLimit)
...
export const validatePostContent = validateContent.label("Content")
